...
**Superseded again 2026-08-16 (same day)** by `.vibe/decisions/022`: Wails dropped too — no webview anywhere. `mode-quick-versus` now uses a from-scratch native Go UI (`go-gl`+SDL2, mirroring Ikemen GO), validated against Ebiten by a dedicated spike (`mode-quick-versus/benchmarks/render-lang-spike/`). `web-ui-kit`/TypeScript is no longer part of the native build at all — web and native are now two separate UI implementations, not one frontend wrapped differently.
