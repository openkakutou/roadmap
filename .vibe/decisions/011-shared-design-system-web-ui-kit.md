...
**Amendment (2026-08-09):** `mode-quick-versus`'s own CLAUDE.md scopes its adoption as "may optionally adopt `web-ui-kit` for non-match UI (selection/setup/result screens); the in-match HUD and rendering are custom game UI, not design-system components" — softer wording than this decision's blanket "every ... app adopts it". Formalizing that as the intended reading rather than a drift: a `mode-*` app's in-match HUD/rendering is real-time game UI with its own performance and layout needs, genuinely different from the panels/forms/dialogs `web-ui-kit` targets — the same distinction decision `004` draws between `engine` (owns match simulation) and everything outside it. The selection/setup/result screens of a `mode-*` app are not exempt: those are exactly the panel/form/dialog UI this decision targets, and still must adopt `web-ui-kit` early per the rule above.
