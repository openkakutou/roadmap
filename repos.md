...
| `character-editor` | planned | Modify and/or create a character: read+write UI on top of `character`. Separate app from `character-viewer-web`, not a successor to it — see `.vibe/decisions/002`. Named `character-editor`, not just `editor` — see `.vibe/decisions/003`. |
| `lifebar-editor` | idea | Editor for MUGEN/Ikemen lifebar files (health/power bar UI config) — distinct scope from `character-editor`. Not yet scoped or committed to. |
