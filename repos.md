...
- [`snd`](https://github.com/openkakutou/snd) — sound (`.snd` v1/v2) parsing, needed independently by `character` (a character's own sounds) and `mode-quick-versus` (system/common sound sets). See `.vibe/decisions/026`.
...
| character | [`character-editor`](https://github.com/openkakutou/character-editor) | active | Modify and/or create a character: read+write UI on top of `character` and `sff`. Separate app from `character-viewer-web`, not a successor to it — see `.vibe/decisions/002`. Named `character-editor`, not just `editor` — see `.vibe/decisions/003`. Ships as a web app (GitHub Pages, live) and, planned, standalone Windows/Mac/Linux desktop builds — see `.vibe/decisions/019`, backlog `007`. |
...
| stage | [`stage-editor`](https://github.com/openkakutou/stage-editor) | active | Modify and/or create a stage: read+write UI on top of `stage`. Incl. Ikemen GO 3D model-based stages, see `.vibe/decisions/014`. Ships as a web app (GitHub Pages, live) and, planned, standalone Windows/Mac/Linux desktop builds — see `.vibe/decisions/019`, backlog `007`. |
...
| lifebar | [`lifebar-editor`](https://github.com/openkakutou/lifebar-editor) | active | Modify and/or create a lifebar: read+write UI, own lifebar parsing (see `.vibe/decisions/009`) plus `sff` for sprite assignment. Ships as a web app (GitHub Pages, live) and, planned, standalone Windows/Mac/Linux desktop builds — see `.vibe/decisions/019`, backlog `007`. |
